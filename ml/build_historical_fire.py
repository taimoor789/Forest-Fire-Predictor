...
from ml import attribution, config, manifest

NFDB_SHP = config.REFERENCE_DIR / "nfdb" / "NFDB_point_20260811.shp"

# Bounds sanity filter -- the raw archive has 243 rows (0.05%) with
# corrupted LATITUDE/LONGITUDE attribute values (e.g. LATITUDE=-115.667,
# LONGITUDE=116188.0), independent of the geometry column. Generous enough
# to keep every real Canadian fire, tight enough to drop the garbage.
...
def load_nfdb():
    ...
    return gdf[["YEAR", "LATITUDE", "LONGITUDE", "SIZE_HA"]].rename(
        columns={"YEAR": "year", "LATITUDE": "lat", "LONGITUDE": "lon", "SIZE_HA": "size_ha"}
    )


def attribute_all_fires(fires: pd.DataFrame, grid_domain: pd.DataFrame):
