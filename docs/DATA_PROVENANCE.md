...
| `data/reference/era5/{hourly,precip}_{year}_part{N}.grib` | 5 | **re-acquiring** — ERA5-Land via Copernicus CDS API, 2019–2023 (raw source files, not manifest-tracked). CaSPAr (the original plan, same model family as production) was unreachable for 10+ hours with no outage info, so ERA5 is the fallback — a different provider than live production, residual bias expected and measurable. Each year is split into `part0`/`part1` (≤150-day chunks): CDS rejects a single ~289-day request outright with "cost limits exceeded" even though it's well under the nominal 12,000-field limit (a 214-day request the same shape had worked earlier), so chunking was required, not just an optimization. `hourly_*`: temp/dewpoint/wind-u/wind-v/pressure at the 5 UTC hours corresponding to local noon somewhere in Canada, covering `config.REPLAY_WARMUP_DAYS`=75 days before the fire season starts (not just the season itself) so Stage 6's replay has real weather to burn DC/BUI in on before the modelling window begins — the first acquisition (since superseded) only covered the season itself with no pre-season warmup, a gap against the pre-registered design caught before Stage 6 was built on it. `precip_*`: total_precipitation at all 24 hourly forecast-steps of each day's accumulation run (needed to reconstruct a rolling 24h sum via `ml/weather/adapter.py`, not a single reading — `tp` resets to ~0 at each day's own 00:00 UTC and accumulates monotonically through step 24, confirmed directly against real data). The CDS request now uses explicit `date` strings rather than separate year/month/day lists — the latter get cross-producted independently by CDS and had silently expanded an earlier request to ~all of March–October instead of the intended range (harmless when it happened — only added extra lead-in days — but fixed going forward). |
| `data/fwi_replay_{year}.parquet` | 6 | **code ready, not yet run** — `ml/replay_fwi.py` written; `test_replay_parity.py` passing (verifies the replay's `bootstrap_from_window`/`advance_one_day`/gap-reinit orchestration reduces to exactly the same result as calling `fire_risk.py` directly, using real `weather_data/*.csv` rows). Blocked on the ERA5 re-acquisition above finishing. |
